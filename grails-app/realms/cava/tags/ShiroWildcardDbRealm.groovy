...
class ShiroWildcardDbPrincipalHolder implements Serializable, PrincipalHolder {
    ...
    Set<String> getRoles() {
        ...
        User user = User.findById(id, [fetch: [roles:"eager"] ])
        Set <String> roles = user.roles?.collect { it.name } ?: []
    }

    Set<String> getPermissions() {

        Set <String> permissions = []
        User user = User.findById(id, [fetch: [permissions:"eager"], readOnly: true])
        if (!user) {
            return permissions
        }
        permissions.addAll(user.permissions*.permissionsString)

        Set<Permission> rolePermissions = UserRoleAssociation.findAll(
                "from UserRoleAssociation as ura left outer join fetch ura.role.permissions where ura.user = :userInstance",
                [userInstance: user],
                [readOnly:true]
        ).collect { it.role.permissions }.unique() as Set
        permissions.addAll(rolePermissions.flatten())
